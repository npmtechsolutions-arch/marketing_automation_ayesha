...
| 23 | analytics | **S3** | `mock_metrics_untokened` (Facebook, YouTube dev tokens) is also re-randomised on **every read**, so a dev's engagement numbers change on each page refresh. Less serious than #9 because it is gated on a placeholder token, but it makes dev analytics unreadable. | Seed on the post id so a given post is at least stable. |
