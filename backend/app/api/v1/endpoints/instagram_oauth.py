...
async def instagram_authorize(
    ...
    # The consent URL, its scopes and the redirect_uri all belong to
    # the provider now -- this route only mints state.
    return {"auth_url": get_provider("instagram").build_authorize_url(state)}
...
async def instagram_callback(
    ...
    try:
        tokens = await get_provider("instagram").exchange_code(code)
    except ProviderAPIError as exc:
        logger.error("Instagram token exchange failed: %s", exc.detail)
